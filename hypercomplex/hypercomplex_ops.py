...
def hypercomplex_exp(input, num_components=8):
    ...
    # norm_v = torch.sqrt(i * i + j * j + k * k + e * e + l * l + m * m + n * n) + 0.0001
    # exp = torch.exp(r)
    ...
    # e = (e / norm_v) * torch.sin(norm_v)
    # l = (l / norm_v) * torch.sin(norm_v)
    # m = (m / norm_v) * torch.sin(norm_v)
    # n = (n / norm_v) * torch.sin(norm_v)
    components[0] = torch.cos(norm_v)
    components[1:] = [(component / norm_v) * torch.sin(norm_v) for component in components[1:]]

    # return torch.cat([exp * r, exp * i, exp * j, exp * k, exp * e, exp * l, exp * m, exp * n], dim=1)
